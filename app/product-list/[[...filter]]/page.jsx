...
// Typy danych (usunięte, ale tablica z nazwami kategorii została)
const productTypes = ['procesor', 'karta graficzna', 'pamięć ram', 'dysk'];

// --- Renderer dla szczegółów pojedynczego produktu (Zad. 6.1/7.1) ---
function renderProductDetails(product) {
    const isAvailable = product.amount > 0;
    
    // Używamy klas CSS z app/global.css
    const stockClass = isAvailable ? 'in-stock' : 'out-of-stock';

    // Trasa do wyświetlenia obrazka (dla zadania 11.2)
    const imageLinkHref = `/product-list/${product.type.replace(/\s/g, '')}/${product.id}/image`;
    ...
    return (
        <div className="product-details-container">
            ...
            <div className="product-details-grid">
                ...
                <div>
                    <h2 style={{ fontSize: '2.5rem', marginTop: 0, color: '#bcbcb7' }}>{product.name}</h2>
...
// --- Renderer dla listy produktów (Zad. 6.1/9.2) ---
function renderProductList(products, title) {
    
    // Style linków kategorii
            ...
            <nav style={{ marginBottom: '20px', border: '1px solid #bcbcb7', padding: '15px', borderRadius: '5px', backgroundColor: 'rgba(255, 255, 255, 0.05)' }}>
                ...
                <ul style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', listStyle: 'none', padding: 0, margin: 0 }}>
                    ...
                    {Array.from(categories).map(cat => (
                        <li key={cat}>
                            {/* Link do filtrowania po kategorii */}
            ...
            <ul className="product-list-grid">
                {products.map((p) => (
                    <li key={p.id} className="product-list-item">
                        ...
                        {/* Link do szczegółów produktu: /product-list/[kategoria]/[id] */}
                        <Link href={`/product-list/${p.type.replace(/\s/g, '')}/${p.id}`} style={{ display: 'block' }}>
...
// --- Główna logika routingu dla segmentu uniwersalnego ---
export default async function DynamicProductPage({ params }) {
    const filterSegments = params.filter;
    
    // Scenariusz 1: Lista wszystkich produktów (/product-list)
    ...
    // Scenariusz 2: Lista produktów po kategorii (/product-list/[kategoria])
    ...
    // Scenariusz 3: Szczegóły produktu (/product-list/[kategoria]/[id])
    if (filterSegments.length === 2) {
        const productId = parseInt(filterSegments[1]);
